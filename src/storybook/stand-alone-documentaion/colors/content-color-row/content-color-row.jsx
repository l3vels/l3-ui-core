...
export const ContentColorRow = ({ colorName }) => {
  const regularStyle = useMemo(
    () => ({
      backgroundColor: ColorUtils.getL3velsColorAsStyle(colorName, ColorUtils.modes.REGULAR)
    }),
    [colorName]
  );
  const hoverStyle = useMemo(
    () => ({
      backgroundColor: ColorUtils.getL3velsColorAsStyle(colorName, ColorUtils.modes.HOVER)
    }),
    [colorName]
  );
  const selectedStyle = useMemo(
    () => ({
      backgroundColor: ColorUtils.getL3velsColorAsStyle(colorName, ColorUtils.modes.SELECTED)
    }),
    [colorName]
  );
