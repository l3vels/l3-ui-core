...
<Canvas>
    <Story name="States" play={statesPlaySuite}>
        <Flex gap={Flex.gaps.MEDIUM}>
            <Clickable className="l3-story-clickable_first-element" onClick={() => alert("clicked")} ariaLabel="clickable button">
                <div>Regular clickable element</div>
            </Clickable>
            <Clickable className="l3-story-clickable_disabled-element" onClick={() => alert("clicked")} disabled ariaLabel="disabled clickable button">
