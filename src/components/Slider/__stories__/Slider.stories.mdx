...
<Meta title="Inputs/Slider - (In Progress)" component={Slider} argTypes={argTypes} />
...
<Canvas>
  <Story name="WithLabels" decorators={[VerticalStories]}>
    ...
    <Slider indicateSelection={true} defaultValue={55} size={Slider.sizes.SMALL} />
...
<Canvas>
  <Story name="Normal with Label">
    <Slider color={Slider.colors.NORMAL} size={Slider.sizes.SMALL} defaultValue={12} sliderLabel="12%" />
    <Slider color={Slider.colors.NORMAL} size={Slider.sizes.MEDIUM} defaultValue={24} sliderLabel="24%" />
    <Slider color={Slider.colors.NORMAL} size={Slider.sizes.LARGE} defaultValue={35} sliderLabel="35%" />
  </Story>
</Canvas>
