...
const ColorPickerItemComponent = ({
  ...
  const colorAsStyle = isMondayColor ? ColorUtils.getL3velsColorAsStyle(color, colorStyle) : color;
  ...
  useEffect(() => {
    ...
    const onHover = e => {
      if (colorStyle === COLOR_STYLES.SELECTED) {
        e.target.style.background = ColorUtils.getL3velsColorAsStyle(color, COLOR_STYLES.REGULAR);
      } else {
        e.target.style.background = ColorUtils.getL3velsColorAsStyle(color, COLOR_STYLES.HOVER);
      }
    };
  ...
  return (
    <Tooltip content={tooltipContent}>
      <li
        className={cx("l3-style-color-item-wrapper", {
