  ...
  (
    ...
    return (
      <div ref={ref} className={classNames} role="banner" aria-label={ariaLabel || "banner"}>
        <div className="l3-alert-banner__inner">
          {children.map((child, index) => {
            ...
            return (
              <div
                key={index}
                className={cx("l3-alert-banner__inner-item", {
                  "l3-alert-banner__inner-item-text": childTypeIsAlertBannerText
                })}
              >
                {childTypeIsAlertBannerText ? <div className="l3-alert-banner__ellipsis">{child}</div> : child}
              </div>
            );
          })}
        </div>
        <div className="l3-alert-banner__close-button-wrapper">
          {isCloseHidden ? null : (
            <Button
              className="l3-alert-banner__alert-banner-close-btn"
              onClick={onClose}
